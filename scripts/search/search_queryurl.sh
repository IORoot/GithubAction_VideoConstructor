...
function do_search()
{
  ...
  echo yt-dlp --cookies $COOKIE_FILE --get-id --playlist-end $COUNT "$QUERY" $FILTER_PARAM
  yt-dlp --cookies $COOKIE_FILE --get-id --playlist-end $COUNT "$QUERY" $FILTER_PARAM > "$TEMP_FILE"
}
